...
> Este documento registra el **avance de implementación** (qué está corriendo en producción vs. qué falta), no el diseño. La documentación canónica del *diseño* del dominio vive en los módulos (`general-module/`, `recoleccion-module/`, `vivero-module/`, `plantacion-module/`) y en `database/00_database_schema.md`. Este archivo reemplaza el flujo anterior de carpeta `tareas/` como fuente del estado de implementación de la integración M2 ↔ M3.
>
...
| Handler atómico de `DESPACHO` automático desde M3 (al guardar `PLANTACION_INICIAL` / `REPOSICION`) | `CLAUDE.md` (histórico: "tarea 03") | ⏳ pendiente |
| Triggers de contadores materializados: `SUBCAMPANIA` (`total_plantado_inicial`, `total_repuesto`, `total_muerto_acumulado`) y `REGISTRO_PLANTACION` (`cantidad_muerta_acumulada`, `cantidad_repuesta_acumulada`) | schema: "materializado por trigger (tarea pendiente)"; `CLAUDE.md` (histórico: "tarea 13 sugerida") | ⏳ pendiente |
...
Este estado se refinará cuando el usuario confirme, pieza por pieza, qué está realmente en producción. La antigua carpeta `tareas/` (que documentaba tareas 01–12 de esta integración) fue retirada del repo el 2026-07-01; su contenido se entrega por separado para un repaso de rescate de conocimiento permanente, y no debe recrearse aquí.
