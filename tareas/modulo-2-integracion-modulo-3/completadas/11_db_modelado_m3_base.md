...
6 migraciones SQL creadas en `Backend-r3foresta/migrations/`, idempotentes, aplicar en orden:
...
- **Triggers de mantenimiento de contadores M3** (`total_plantado_inicial`, `total_repuesto`, `total_muerto_acumulado` en `SUBCAMPANIA`; `cantidad_muerta_acumulada`, `cantidad_repuesta_acumulada` en `REGISTRO_PLANTACION`) → **tarea 13 sugerida**. Hasta entonces los contadores quedan en 0 y queries M3 que los lean serán incorrectas.
- **Materializar los FKs físicos pendientes** entre M2 y M3 (`asignacion.subcampania_id`, `evento_lote_vivero.subcampania_id|campania_id|registro_plantacion_id`): el ER lógico ya los muestra, pero el `ALTER TABLE` físico no se aplicó. Agrupar con tarea 13 o micro-tarea independiente.
- **`CAMPANIA_HISTORIAL` y `SUBCAMPANIA_HISTORIAL`** (eventos de ciclo de vida listados en doc M3 §4.1) → tarea posterior.
- **Job nocturno** de transición `MANTENIMIENTO_ACTIVO → MONITOREO_HISTORICO` cuando `today >= fecha_fin_mantenimiento` → tarea posterior.
- **Eliminación del boceto legacy `PLANTACION` + `PLANTACION_*`** de la BD (nunca poblaron datos, no hay consumidores) → **[tarea 12](../12_db_drop_legacy_plantacion.md) creada**.

### Riesgos validados antes de aplicar

1. **PostGIS** habilitado en Supabase (verificar con `select postgis_version()`).
2. **`organizacion` real**: si el módulo General materializa su versión definitiva, eliminar el placeholder de 028 y ajustar.
3. **Tarea 10 aplicada** antes (decisiones de docs cerradas: sin mix de especies, coordinador como membresía, etc.).
4. **Contadores en 0** hasta que la tarea 13 (triggers) se aplique.

### Referencias

- Commits: `bf61933 tarea 11 completada`, `2fab2c8 # Informe — Tarea 11 (modelado base M3)`.
- Migraciones aplicadas: `Backend-r3foresta/migrations/027` a `032`.
- Tareas desbloqueadas: [tarea 03](../03_backend_despacho_automatico_atomico.md) (despacho automático atómico).
- Tarea derivada: [tarea 12](../12_db_drop_legacy_plantacion.md) (drop legacy PLANTACION).
- Propagación canónica: `database/00_database_schema.md` actualizado con las 9 entidades M3, 5 enums nuevos, vista `campania_estado` y función `gps_dentro_poligono_con_tolerancia`.
