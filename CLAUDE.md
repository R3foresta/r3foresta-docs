...
- **Plantación — Edición básica y desactivación de campaña en MVP.** `CAMPANIA.nombre`, `descripcion`, fechas estimadas y `CAMPANIA_ORGANIZACION` pueden corregirse como datos generales. `CAMPANIA.tipo` solo se edita si no existe ninguna `SUBCAMPANIA` asociada. La campaña puede desactivarse/inactivarse si no tiene subcampañas o si todas estan `CANCELADA`; `DELETE` físico solo si no tiene subcampañas. La edición flexible de `tipo` queda post-MVP.
