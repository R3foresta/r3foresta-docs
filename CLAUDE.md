...
- **`03-plantacion-module/`** — Módulo 3. Modelado base en BD (CAMPANIA → SUBCAMPANIA → REGISTRO_PLANTACION → EVENTO_PLANTACION) aplicado vía migraciones 027–032. La integración física M2↔M3 y la desactivación atómica de campañas están implementadas mediante migraciones 051–057. El 2026-07-23 se confirmó específicamente que `056` y `057` están aplicadas en Supabase de producción y que Backend/Frontend están desplegados. Quedan flujos no incluidos como el job nocturno de transición a `MONITOREO_HISTORICO`. Ver [ESTADO.md](ESTADO.md).
...
- **Vivero ↔ Plantación — asignación es entrega física:** crear una `ASIGNACION_VIVERO_SUBCAMPANIA` significa entregar plantas reales a una subcampaña. Debe generar una salida en `EVENTO_LOTE_VIVERO`, descontar `LOTE_VIVERO.saldo_vivo_actual` y crear stock consumible en la subcampaña (`saldo_asignado_disponible`). Plantar en M3 consume ese stock; no baja de nuevo el lote.
...
- **Vivero ↔ Plantación — devolución física:** devolver una asignación ya no libera una reserva lógica; retorna plantas físicas al vivero. En MVP se registra cantidad y motivo, sin fotos obligatorias, aumenta `cantidad_devuelta` y debe aumentar `LOTE_VIVERO.saldo_vivo_actual` del lote origen.
- **Vivero ↔ Plantación — mermas:** una `MERMA` de vivero solo afecta plantas que siguen físicamente en vivero. Las asignaciones ya entregadas no son afectadas por mermas posteriores del lote; si se necesita pérdida de stock en campo, debe afectar `cantidad_mermada` de la asignación con un flujo M3 separado.
- **Despacho a subcampaña vs manual:** una salida hacia subcampaña ocurre al crear la asignación física y debe usar un origen específico como `ASIGNACION_SUBCAMPANIA`, con `destino_tipo = PLANTACION_CAMPANIA`, `subcampania_id` y `campania_id`. `registro_plantacion_id` debe quedar `NULL` porque aún no existe una plantación. No usar `AUTOMATICO_PLANTACION` para el flujo vigente.
- **Despacho manual valida contra saldo físico:** los despachos manuales validan cantidad contra `LOTE_VIVERO.saldo_vivo_actual`. Como la asignación física ya descontó el lote, no hay stock reservado dentro del vivero que restar.
...
- **Plantación — Edición básica y desactivación de campaña en MVP.** `CAMPANIA.nombre`, `descripcion`, fechas estimadas y `CAMPANIA_ORGANIZACION` pueden corregirse como datos generales. `CAMPANIA.tipo` solo se edita si no existe ninguna `SUBCAMPANIA` asociada. Hay dos acciones explícitas: desactivación estricta si no hay subcampañas vivas no canceladas, y desactivación atómica con cancelación masiva de `BORRADOR`/`ACTIVA` sin plantaciones, devolución física de stock y soft-delete de campaña. Una hija con plantaciones o estado no elegible bloquea y revierte toda la operación. `DELETE` físico solo si no existe ninguna subcampaña. La edición flexible de `tipo` queda post-MVP.
- **Plantación — metas por especie en subcampaña.** `CAMPANIA` no tiene meta operativa propia **persistida** (sin columna de meta); la meta vive en `SUBCAMPANIA`. Cada subcampaña debe poder planificar su composición en `SUBCAMPANIA_META_ESPECIE`: porcentaje objetivo y cantidad objetivo por `planta_id`, con suma de porcentajes = 100 y suma de cantidades = `SUBCAMPANIA.meta_total_arboles` al activar. La asignación de vivero es stock físico por lote ya entregado a la subcampaña; el plan por especie vive en M3 y se usa para cobertura, progreso y validación de `PLANTACION_INICIAL`. Para planeación, la campaña expone un total **derivado** `meta_planificada_campania` = suma de `meta_total_arboles` de las subcampañas no `CANCELADA` (incluye `BORRADOR`), calculado al leer y nunca materializado como columna (`RN-PLA-36`).
- **Plantación — La meta es planeación; la asignación es cumplimiento.** El plan de metas (total y por especie) se define en `BORRADOR` y habilita la activación; las asignaciones de lotes de vivero **no ocurren en `BORRADOR`** sino tras `ACTIVA` (`RF-VIV-11`). Por tanto una subcampaña puede activarse con 0% de stock asignado y asignar después; no se exige ninguna asignación para activar.
- **Plantación — Cancelación solo sin plantaciones (`RN-PLA-37`).** Una subcampaña pasa a `CANCELADA` únicamente si no tiene ninguna `PLANTACION_INICIAL` (`total_plantado_inicial = 0`): aplica a `BORRADOR` y a `ACTIVA` sin plantar. Si ya se plantó algo, el cierre anticipado es `FINALIZADA_PARCIAL`, no cancelación. Cancelar conserva el registro (inactivación, no borrado), registra `SUBCAMPANIA_CANCELADA`, saca su meta del agregado de campaña y resuelve asignaciones activas con devolución física si ya fueron entregadas. `CANCELADA` deja de ser estado reservado; `PAUSADA` sigue reservado.
...
- **Integración M2 ↔ M3:** contrato separado en [`90-contratos-integracion/02_contrato_vivero_a_plantacion.md`](90-contratos-integracion/02_contrato_vivero_a_plantacion.md). Asignación física, consumo por plantación/reposición, devolución física, cancelación con retorno de stock, constraint de incremento por devolución, saldos físicos, merma M2 separada y desactivación atómica de campaña están implementados mediante migraciones 051–057. El 2026-07-23 se confirmó la aplicación productiva de `056`/`057`, las verificaciones y el despliegue coordinado de Backend/Frontend. Ver [ESTADO.md](ESTADO.md).
