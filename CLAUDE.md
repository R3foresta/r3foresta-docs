...
- **Vivero ↔ Plantación — mermas LIFO sobre asignaciones:** una `MERMA` que excede el saldo no asignado del lote distribuye el excedente sobre asignaciones activas en orden LIFO (`fecha_asignacion DESC, id DESC` — la más nueva primero), sin modificar `cantidad_asignada`. Razón: la asignación más reciente corresponde a la plantación más lejana en el tiempo, que tiene mayor margen para reorganizarse.
