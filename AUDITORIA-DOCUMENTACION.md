...
- **General (`general-module/`, `RF-GEN-*`)** — catálogos y entidades maestras transversales: `USUARIO`, `UBICACION`/territorios (`PAIS`, `DIVISION_ADMINISTRATIVA`), `VIVERO`, `PLANTA`/`TIPO_PLANTA`, `EVIDENCIAS_TRAZABILIDAD`, `METODO_RECOLECCION`. Es la fuente viva de identidad; los demás módulos la consumen y congelan *snapshots*.
- **Recolección — Módulo 1 (`recoleccion-module/`, `RF-REC-*`)** — registro del *lote origen* con evidencia fotográfica, GPS y validación. Doble estado: registro (`BORRADOR → PENDIENTE_VALIDACION → VALIDADO/RECHAZADO`) e inventario (`ABIERTO/CERRADO`). Persistencia multicapa (Supabase DB + Storage + IPFS/Pinata + blockchain).
- **Vivero — Módulo 2 (`vivero-module/`, `RF-VIV-*`, `RN-VIV-*`)** — maduración pre-plantación. Modelo **híbrido** (estado actual + historial append-only de eventos: `INICIO → EMBOLSADO → (MERMA | DESPACHO | ADAPTABILIDAD)* → CIERRE`). El *lote de vivero* es el agregado central con **origen único**.
- **Plantación — Módulo 3 (`plantacion-module/`, `RF-PLA-*`)** — plantación efectiva en campo. Arquitectura de dos niveles: `CAMPANIA` (estratégica, estado derivado) → `SUBCAMPANIA` (operativa, con polígono, meta, equipo). Incluye mantenimiento (mortandad + reposición) y vista pública.
...
3. **`vivero-module/01_regas_de_negocio_vivero.md`** — reglas `RN-VIV-01..60`; incluye el contrato M2↔M3 formal.
4. Los **`00_Requerimientos_*.json`** y **`01_reglas_*.md`** de cada módulo, más **`plantacion-module/02_Procesos_Modulo_3_Plantacion.md`**.
...
r3foresta-docs/
├─ README.md, CLAUDE.md
├─ general-module/     README + 00_json + 01_reglas + 02_guia + 03_decisiones + agregar.md
├─ recoleccion-module/ README + 00_json + 01_reglas + 02_procesos + 03_diagrama + 04_operativo
├─ vivero-module/      00_json + 01_reglas + 02_guia + 03_addendum + 03_operativo + 04_consumo + image.png
├─ plantacion-module/  00_json + 02_procesos + 03_mockups        (sin 01_reglas, sin README)
...
| **READMEs por módulo** | ⚠️ Incompleto | Solo `general-module/` y `recoleccion-module/` tienen README. **Faltan** en `vivero-module/`, `plantacion-module/` y `database/`, aunque `CLAUDE.md` dice "README.md por módulo". |
...
- **`README.md` raíz** (hecho): (a) contiene **2 rutas absolutas de máquina** `/Users/pabloandresfernandezcari/...` como enlaces — no portables, se rompen en cualquier otra máquina; (b) **no menciona `plantacion-module/`** (el Módulo 3, que sí existe y está muy desarrollado); (c) enlaza general y recolección pero deja vivero y database como texto plano.
- **Formato heterogéneo de requerimientos** (hecho): `general`, `recoleccion` y `plantacion` usan `{"requerimientos":[...]}`; `vivero-module/00_...json` usa un **array plano** con claves distintas (`"Severidad"`, `"Aplica en MVP"`, `"Aporta a carbono"`). Dos "esquemas" de JSON conviviendo.
- **Reglas de M3 alojadas en M2** (hecho): plantación no tiene `01_reglas`; sus reglas de integración viven como `RN-VIV-47..60` dentro de `vivero-module/01_regas_de_negocio_vivero.md`, y otras decisiones (p. ej. `RN-VIV-60` reposición de especie libre) rigen comportamiento de M3.
...
| 3 | `general-module/README.md` | Propósito y dependencias del módulo | Alta | Completo | Alta | Sí | Mantener (añadir `ORGANIZACION`) |
| 4 | `general-module/00_Requerimientos_Modulo_General.json` | RF-GEN-01..06 | Alta | Completo | Alta | Sí | Mantener |
| 5 | `general-module/01_reglas_de_negocio_general.md` | RN-GEN-01..26 | Alta | Completo | Alta | Sí | Mantener |
| 6 | `general-module/02_guia_operativa_modulo_general.md` | Cómo administrar catálogos | Media | Completo | Alta | Sí | Mantener |
| 7 | `general-module/03_decisiones_pendientes_general.md` | Decisiones de MVP | Media | Completo | Alta | Parcial | **Depurar/renombrar**: el título dice "pendientes" pero el contenido está **cerrado** |
| 8 | `general-module/agregar.md` | Nota suelta sobre evidencia/compresión | Baja | Parcial | Media | Parcial (2026-06-23, el más reciente) | **Fusionar** en reglas de evidencia; nombre genérico sin prefijo |
| 9 | `recoleccion-module/README.md` | Resumen del flujo M1 | Media | Parcial | Media | Sí | Mantener; sincronizar cifra de fotos |
| 10 | `recoleccion-module/00_Requerimientos_Moduulo _1_Recoleccion.json` | RF-REC-01..05 | Alta | Completo | Alta | Sí | Mantener; **renombrar** archivo (typo + espacio) |
| 11 | `recoleccion-module/01_regas_de_negocio_recoleccion.md` | RN-REC-01..27 | Alta | Completo | Alta | Sí | Mantener; renombrar (typo "regas") |
| 12 | `recoleccion-module/02_Procesos_Modulo_1_Recolección.md` | Proceso M1 detallado | Alta | Completo | Alta | Sí | Mantener |
| 13 | `recoleccion-module/03_procesos_diagrama.md` | 4 diagramas Mermaid de M1 | Media | Completo | Alta | Sí | Mantener |
| 14 | `recoleccion-module/04_operativo_modulo_recoleccion.md` | Resumen operativo corto M1 | Media | Completo | Alta | Sí | Mantener (solapa parcial con 02) |
| 15 | `vivero-module/00_Requerimientos-Modulo_2_Vivero.json` | RF-VIV-01..14 | Alta | Completo | Media-Alta | Sí | Mantener; corregir "LIFO" en RF-VIV-03/14 |
| 16 | `vivero-module/01_regas_de_negocio_vivero.md` | RN-VIV-01..60 (incl. contrato M3) | Alta | Completo | **Alta** (fuente correcta del criterio de merma) | Sí | **Mantener** — candidato a fuente de verdad de reglas |
| 17 | `vivero-module/02_doc_guia_viviero.md` | Guía operativa M2 + §13 integración | Alta | Completo | Media | Sí | Mantener; renombrar (typo); resolver contradicción interna LIFO/urgencia |
| 18 | `vivero-module/03_Addendum_Modulo_2_por_Modulo_3.md` | Contrato negociado M2↔M3 | Media | Completo | Media | No (autodeclarado "ABSORBIDO") | **Archivar** a histórico/decisiones |
| 19 | `vivero-module/03_operativo_modulo_vivero.md` | Resumen operativo corto M2 (pre-M3) | Media | Parcial | Media-Baja | Parcial | **Fusionar** con 04_consumo; enum `destino_tipo` viejo |
| 20 | `vivero-module/04_consumo_de_vivero.md` | Operativo del consumo M2 (con M3) | Alta | Completo | Media-Alta | Sí | Mantener como operativo único de consumo |
| 21 | `vivero-module/image.png` | Imagen sin referencia hallada | Baja | — | — | No | **Depurar** (sin uso aparente en los `.md`) |
| 22 | `plantacion-module/00_Requerimientos_Modulo_3_Plantacion.json` | RF-PLA-01..17 | Alta | Completo | **Media** (contiene ítems desactualizados) | Parcial | **Depurar**: RF-PLA-17 y RF-PLA-04 contradicen decisiones cerradas |
| 23 | `plantacion-module/02_Procesos_Modulo_3_Plantacion.md` | Proceso M3 detallado | Alta | Completo | Alta | Sí | **Mantener** — mejor doc de M3 |
| 24 | `plantacion-module/03_Mockups_Guia_Modulo_3_Plantacion.md` | Guía UX/mockups M3 | Media | Completo | Media | Parcial | Depurar: muestra "mix de especies con topes %", que está **fuera del MVP** |
...
- **Fuente correcta (actual):** `RN-VIV-50` en `vivero-module/01_regas_de_negocio_vivero.md` — el excedente se distribuye ordenando por `subcampania.fecha_estimada_inicio DESC NULLS FIRST` (la subcampaña que empieza **más tarde** absorbe primero; la más urgente queda protegida). Coincide con `CLAUDE.md` (invariante), `plantacion §9.4`, `04_consumo §4` y `addendum §7`.
...
- **Obsoleta (3 términos, sin `cantidad_mermada`):** `plantacion-module/00_...json:132` (RF-PLA-04) y `plantacion §8.1`. Los docs de M3 quedaron atrás porque se escribieron antes de la política de merma-sobre-asignación (RF-VIV-14).
...
- **Obsoleto:** `plantacion-module/00_...json:500` (RF-PLA-17) dice *"COORDINADOR es un rol nuevo a crear en el enum rol_usuario"*. Contradice directamente el esquema y `CLAUDE.md`.
...
`plantacion-module/00_...json` contiene conocimiento **congelado antes** de decisiones cerradas: RF-PLA-17 (COORDINADOR al enum, §4.3) y RF-PLA-04 (fórmula de 3 términos, §4.2). Es el caso de libro de **"un documento existe desactualizado porque otro se actualizó y este no"**: el MD de procesos y `CLAUDE.md` se corrigieron; el JSON no.

### 5.5. Notas sueltas e histórico incrustado

- `general-module/agregar.md`: nota de política de compresión de evidencia, sin prefijo numérico, con una sección "Pendiente para M3". Debería vivir dentro de las reglas de evidencia (RN-GEN-21..23), no como archivo aparte.
- `general-module/03_decisiones_pendientes_general.md`: el nombre dice "pendientes" pero **todo está cerrado** ("decisión tomada", "quedan cerrados para el MVP"). El título miente sobre el contenido.
...
- `vivero-module/image.png`: sin referencia hallada en los `.md`.
...
5. **Documento del maestro `ORGANIZACION` en `general-module/`** — el esquema mismo marca `%% Placeholder defensivo (modulo General aun no la define)`. Es una entidad maestra documentada solo en M3.
...
7. **`plantacion-module/01_reglas_de_negocio_plantacion.md` + README del módulo** — para que M3 no dependa de reglas alojadas en M2.
...
FUNDACIONAL
  ...
  general-module/ (RF-GEN, RN-GEN) ← maestros: identidad, territorios, evidencia, ORGANIZACION*

        │ (consumen maestros y congelan snapshots)
        ▼
MÓDULOS OPERATIVOS
  recoleccion-module  ──contrato M1↔M2──►  vivero-module  ──contrato M2↔M3──►  plantacion-module
...
├─ general-module/  README + 00_requerimientos + 01_reglas + 02_guia + 03_organizacion(nuevo)
├─ recoleccion-module/ README + 00_requerimientos + 01_reglas + 02_procesos + 03_diagrama
├─ plantacion-module/ README(nuevo) + 00_requerimientos + 01_reglas(nuevo) + 02_procesos + 03_mockups
└─ vivero-module/   README(nuevo) + 00_requerimientos + 01_reglas + 02_procesos + 03_consumo(operativo único)
```
...
| Maestros (incl. `ORGANIZACION`) | `general-module/` | M1, M2, M3 |
...
- **Acción:** índice navegable sin rutas absolutas, incluyendo `plantacion-module/` y `database/`; crear README en vivero, plantación y database.
...
- **Acción:** documentar `ORGANIZACION` en general; crear `plantacion-module/01_reglas` (moviendo/espejando las `RN` de M3 que hoy viven en M2, con numeración estable).
