-- 055_vivero_merma_fisica.sql
-- Tarea M2-M3-05: la merma M2 vuelve a ser un evento puramente FISICO del lote.
--
-- Bajo el contrato de asignacion fisica (051+), el stock asignado a subcampanias
-- ya NO esta en el vivero: una merma del lote no puede afectar asignaciones
-- entregadas (contrato seccion 8.1). Esta migracion reemplaza la version LIFO
-- de la 036:
--   - valida solo contra LOTE_VIVERO.saldo_vivo_actual,
--   - no lee ni modifica ASIGNACION_VIVERO_SUBCAMPANIA,
--   - no genera metadata de afectacion de asignaciones.
--
-- La merma de stock asignado en campo (contrato seccion 8.2) queda FUERA del
-- MVP; si se implementa, vive en M3 y afecta cantidad_mermada de la asignacion.
--
-- Depende de: 051 (contrato fisico). Reemplaza el cuerpo definido en 036.
...
BEGIN
  ...
  -- -----------------------------------------------------------------------
  -- 8. Validar contra el saldo vivo FISICO del lote.
  --    El stock asignado a subcampanias ya salio del vivero: la merma del
  --    lote NUNCA lee ni modifica asignaciones (contrato 8.1).
  -- -----------------------------------------------------------------------
  ...
  IF p_cantidad_perdida > v_saldo_antes THEN
    RAISE EXCEPTION
      'La merma (%) no puede exceder el saldo vivo fisico (%) del lote %.',
      p_cantidad_perdida,
      v_saldo_antes,
      p_lote_id;
  END IF;
  ...
  -- -----------------------------------------------------------------------
  -- 11. Insertar evento MERMA (sin metadata de afectacion de asignaciones)
  -- -----------------------------------------------------------------------
...
COMMENT ON FUNCTION public.fn_vivero_registrar_merma(
  BIGINT,
  DATE,
  BIGINT,
  INTEGER,
  ...
  'Registra una MERMA fisica del lote de vivero. Valida solo contra LOTE_VIVERO.saldo_vivo_actual y NUNCA lee ni modifica asignaciones a subcampanias: el stock asignado ya salio fisicamente del vivero (contrato M2-M3 seccion 8.1).';
